function colorModeToggle() {
  ...
  function setColors(colorObject, animate) {
    ...
    //Set the svg logo on the toggle button.
    if (colorObject === secondaryColors) {
      document.querySelectorAll(`.${toggleButton}`).forEach((el) => {
        el.innerHTML = secondaryLogo
      })
    }
    else {
      document.querySelectorAll(`.${toggleButton}`).forEach((el) => {
        el.innerHTML = primaryLogo
      })
    }
  }
